...
# FIX: bump this whenever DEFAULT_KEYMAP's values change. Without a
# version check, an existing keymap.json only ever gets NEW keys filled
# in — it never updates a key that already exists, so every past default
# change silently kept getting overridden by whatever was saved from an
# earlier run/version, even though the user never touched that key
# themselves. On a version bump, the whole file resets to the current
# defaults (customization is still new/actively developed right now, so
# this is the right tradeoff for now — once this stabilizes for real
# distribution, this could be made to only reset genuinely stale
# defaults rather than everything).
KEYMAP_VERSION = 2
    ...
    try:
        if os.path.exists(path):
            ...
            with open(path, 'r') as f:
                loaded = json.load(f)
            saved_version = loaded.get('_version', 0)
            if saved_version < KEYMAP_VERSION:
                print('KEYMAP: saved file is version %r, current is %r — '
                      'resetting to current defaults' %
                      (saved_version, KEYMAP_VERSION))
                km = dict(DEFAULT_KEYMAP)
                _write_keymap(km)
                return km
...
def _write_keymap(km):
    data = dict(km)
    data['_version'] = KEYMAP_VERSION
...
def draw_icon(c, key, cx, cy, sz):
    ...
    with c:
        ...
        if key == 'CAR':
            ...
            Line(points=[cx-rw/2+12*s, cy+10*s, cx-rw/2+24*s, cy+10*s+rh], width=lw*0.6)
            ...
            Line(points=[cx-bw/2+8*s, cy+bh/2-4*s, cx+bw/2-8*s, cy+bh/2-4*s], width=lw*0.55)
            win_w, win_h = 18*s, 14*s; win_y = cy+8*s
            for i in range(3):
            ...
            Line(points=[cx+bw/2-15*s, cy-bh/2+4*s, cx+bw/2-15*s, cy-bh/2+26*s], width=lw*0.55)
            ...
            cab_w, cab_h = 32*s, 46*s; bod_w, bod_h = 64*s, 30*s
            ...
            Line(points=[cx-cab_w/2+4*s, cy-bod_h/2+3*s, cx-cab_w/2+4*s, cy+2*s], width=lw*0.55)
            ...
            cab_w, cab_h = 28*s, 52*s; bod_w, bod_h = 96*s, 28*s
            Line(rectangle=(cx-cab_w/2-bod_w, cy-bod_h/2, bod_w, bod_h), width=lw)
            for i in range(1, 4):
            ...
            Line(points=[cx-cab_w/2+4*s, cy-bod_h/2+3*s, cx-cab_w/2+4*s, cy+3*s], width=lw*0.55)
            Line(rounded_rectangle=(cx+cab_w/2-5*s, cy-bod_h/2+2*s, 4*s, 14*s, 1*s), width=lw*0.7)
            front_x = cx-wr; mid_x = cx-cab_w/2-bod_w*0.45-wr; rear_x = cx-cab_w/2-bod_w+14*s-wr
...
class IconButton(Button):
    ...
    def _redraw(self, *a):
        ...
        with self.canvas.before:
            Color(*bg)
            RoundedRectangle(pos=self.pos, size=self.size, radius=[self.CORNER])
        self._draw_icon()
    ...
    def __init__(self, **kwargs):
        ...
        for path, label in ((src_play, 'play'), (src_pause, 'pause')):
            if not os.path.exists(path):
                print('ICON', label, path, 'MISSING — place a 256x256 RGBA PNG there')
...
class UndoRedoButton(IconButton):
    ...
    def _draw_icon(self):
        ...
        pos = (self.x + (w - sz) / 2, self.y + (h - sz) / 2)
        if self._img_undo is not None:
            self._img_undo.size = (sz, sz); self._img_undo.pos = pos
        if self._img_redo is not None:
            self._img_redo.size = (sz, sz); self._img_redo.pos = pos
...
class LockButton(IconButton):
    ...
    def _draw_icon(self):
        ...
        pos = (self.x + (w - sz) / 2, self.y + (h - sz) / 2)
        if self._img_lock:
            self._img_lock.size = (sz, sz); self._img_lock.pos = pos
        if self._img_unlock:
            self._img_unlock.size = (sz, sz); self._img_unlock.pos = pos
        ...
        with self.canvas.after:
            ...
            if self._is_locked:
                ...
                Line(circle=(cx + 14*s, cy + 10*s, 14*s, 60, 180), width=lw, cap='round')
...
class SquareVehicleButton(Button):
    ...
    def _arm_timeout(self):
        self._cancel_timeout()
        self._timeout_ev = Clock.schedule_once(self._force_release, self.PRESS_TIMEOUT)
...
class TimerWidget(BoxLayout):
    ...
    def _open_set(self, *a):
        ...
        inp_m = _inp("MM"); inp_s = _inp("SS")
        colon = Label(text=":", font_size=36, bold=True, color=(1, 1, 1, 1),
                      size_hint=(None, 1), width=28)
        time_row.add_widget(inp_m); time_row.add_widget(colon); time_row.add_widget(inp_s)
        ...
        btns.add_widget(cancel); btns.add_widget(confirm)
        ...
        def _apply(*a):
            try:
                ...
                s_val = int(inp_s.text.strip()) if inp_s.text.strip() else prev_s
    ...
    def _show_keybindings_editor(self):
        ...
        def _highlight_editor():
            for i, stop in enumerate(tab_stops):
                _widget_for_stop(stop).opacity = 1.0 if i == selected['idx'] else 0.55
        ...
        def _editor_keydown(window, key, scancode, codepoint, modifiers):
            ...
            if key == 9:  # Tab (Shift+Tab goes backward)
                ...
                _highlight_editor()
                try:
                    scroll.scroll_to(_widget_for_stop(tab_stops[selected['idx']]))
                except Exception:
                    pass
                return True
            if key in (13, 271):  # Enter / numpad Enter
                _widget_for_stop(tab_stops[selected['idx']]).dispatch('on_release')
                return True
...
class TrafficCounterApp(App):
    def build(self):
        ...
        if platform == 'android':
            ...
            # FIX: Window.maximize() was sizing to the FULL monitor
            # resolution rather than the work area (screen minus
            # taskbar), covering the taskbar even though the window
            # border/buttons were still showing normally (not truly
            # fullscreen, just wrongly sized). Explicitly querying and
            # using the actual Windows work area fixes this properly.
            sized = False
            if os.name == 'nt':
                try:
                    import ctypes
                    ...
                    Window.size = (rect.right - rect.left, rect.bottom - rect.top)
                    Window.left, Window.top = rect.left, rect.top
                    sized = True
                except Exception as e:
                    print('WORK AREA sizing failed (%s), falling back to maximize()' % e)
            if not sized:
                Window.maximize()
