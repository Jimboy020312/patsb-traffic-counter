...
# FIX (per request): the right grid is now a left-right mirror of the
# left grid — only the RIGHT cluster's layout changed, the left grid
# above is untouched. Each row's two columns are swapped, so what was on
# the left of a row is now on the right and vice versa. Note this also
# moves the None slot (where SquareGridCluster places the Start/Pause
# button for corner=='right') to the top-left cell of the right cluster.
GRID_KEYS_RIGHT = [
    [None,  "LRY"],
    ["CAR", "MOTO"],
    ["BUS", "LLRY"],
...
class TimerWidget(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # second Set-Timer popup underneath the first.
...
class RootLayout(FloatLayout):
    ...
    def _build_shortcuts_bar_text(self):
        # FIX (per request): removed the per-vehicle increment key help
        # (the J1/J2 Car/Moto/Lorry/Bus/LLorry segments) from this bar —
        # it stays translucent-strip only, no vehicle key labels. The
        # remaining entries are reordered left-to-right as requested:
        # Decrement, Esc, Space, Timer, Lock, Pin, then F1.
        km = self.keymap
        return (
            "F1 = Help / Customize   |   Shift = Decrement   |   Esc = Reset All   |   "
            "{pause} = Pause   |   {set} = Timer   |   "
            "{lock} = Lock   |   {pin} = Pin"
        ).format(
...
class TrafficCounterApp(App):
    ...
    # "Settings" panel on F1 (it listens on the older on_keyboard event,
    # so it fires regardless of what our own handler does or returns).
    # That's the unexplained popup on F1/Ctrl+F1. Turning this off is the
    # supported way to disable it.
    ...
    def build(self):
        ...
        if platform == 'android':
            ...
            # FIX: reverted the earlier SystemParametersInfoW work-area
            # sizing attempt — it set Window.size/Window.top directly
            # without accounting for the OS window decorations (title
            # bar, borders), so the title bar ended up pushed off-screen
            # and the window looked fullscreen even though it technically
            # wasn't. A plain Window.maximize() delegates to the OS's own
            # real "maximize" behaviour (same as double-clicking the
            # title bar) — title bar, minimize/maximize/restore/close
            # buttons, and the Windows taskbar all stay visible.
            Window.maximize()
